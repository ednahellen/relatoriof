...
namespace GPSFA_WinForms
{
    public partial class frmEstoque : Form
    {
        ...
        private bool CriarBackupAntesImportacao(string nomeArquivo)
        {
            try
            {
                string timestamp = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                string nomeBackup = $"tbProdutos_BACKUP_{timestamp}";

                using (var conn = DataBaseConnection.OpenConnection())
                {
                    // 1. Criar tabela de backup
                    string sqlCriarBackup = $"CREATE TABLE IF NOT EXISTS {nomeBackup} AS SELECT * FROM tbProdutos";
                    using (var cmd = new MySqlCommand(sqlCriarBackup, conn))
                    {
                        cmd.ExecuteNonQuery();
                    }
                    ...
                    // 2. Registrar no log
                    string sqlLog = @"
                INSERT INTO tbLogBackups (dataBackup, tabela, descricao, arquivo) 
                VALUES (NOW(), @tabela, @descricao, @arquivo)";

                    using (var cmd = new MySqlCommand(sqlLog, conn))
                    {
                        cmd.Parameters.AddWithValue("@tabela", nomeBackup);
                        cmd.Parameters.AddWithValue("@descricao", $"Backup antes da importação do arquivo: {nomeArquivo}");
                        cmd.Parameters.AddWithValue("@arquivo", nomeArquivo);
                        cmd.ExecuteNonQuery();
                    }
                }
            ...
            {
                MessageBox.Show($"Aviso: Backup automático falhou.\nErro: {ex.Message}",
        ...
        private void BtnImportar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string dataBackup = DateTime.Now.ToString("yyyyMMdd_HHmmss");
                string nomeBackup = $"tbProdutos_BACKUP_{dataBackup}";

                using (var connBackup = DataBaseConnection.OpenConnection())
                {
                    try
                    {
                        string sqlBackup = $"CREATE TABLE {nomeBackup} AS SELECT * FROM tbProdutos";
                        using (var cmd = new MySqlCommand(sqlBackup, connBackup))
                        {
                            cmd.ExecuteNonQuery();
                        }
                        MessageBox.Show($"✅ Backup criado: {nomeBackup}\nData: {DateTime.Now:dd/MM/yyyy HH:mm:ss}",
                            "Backup Automático", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"⚠️ Aviso: Não foi possível criar backup automático.\nErro: {ex.Message}\n\nA importação continuará normalmente.",
                            "Backup não realizado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                }

                // ===== CONTINUAÇÃO DA IMPORTAÇÃO ORIGINAL =====
                ...
                using (var stream = File.Open(ofd.FileName, FileMode.Open, FileAccess.Read))
                {
                    using (var reader = ExcelReaderFactory.CreateReader(stream))
                    {
                        ...
                        // Carregar produtos do banco em cache
                        ...
                        using (var trans = conn.BeginTransaction())
                        {
                            try
                            {
                                MessageBox.Show($"Total de linhas no Excel: {dt.Rows.Count}");
                                ...
                                for (int i = 0; i < dt.Rows.Count; i++)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        string dataEntradaStr = row[2]?.ToString()?.Trim();
                                        string produto = row[3]?.ToString()?.Trim();
                                        string quantidadeStr = row[4]?.ToString()?.Trim();
                                        string origem = row[5]?.ToString()?.Trim();

                                        if (produto == "Produto" || produto.Contains("TOTAL") || produto.Contains("SUM"))
                                            continue;

                                        if (string.IsNullOrEmpty(produto) || string.IsNullOrEmpty(quantidadeStr))
                                            continue;
                                        ...
                                        DateTime validade = DateTime.Now.AddMonths(6);

                                        quantidadeStr = quantidadeStr.Replace(",", ".").Trim();
                                        if (quantidadeStr.EndsWith(".0"))
                                            quantidadeStr = quantidadeStr.Replace(".0", "");
                                        ...
                                        {
                                            erros.AppendLine($"Linha {i}: Quantidade inválida '{quantidadeStr}'");
                                            continue;
                                        }
                                        ...
                                        if (codList == 0)
                                        {
                                            ...
                                            erros.AppendLine($"Produto não encontrado: {produto}");
                                            continue;
                                        }
                                    ...
                                    {
                                        registrosErro++;
                                        erros.AppendLine($"Erro na linha {i}: {ex.Message}");
                                    }
                                }
                                trans.Commit();
                                ...
                                MessageBox.Show(mensagem, "Resultado da Importação", MessageBoxButtons.OK,
                                    registrosErro > 0 ? MessageBoxIcon.Warning : MessageBoxIcon.Information);
                            }
                            catch (Exception ex)
                            {
                                trans.Rollback();
                                MessageBox.Show($"❌ Erro durante a importação: {ex.Message}");
                            }
                        }
                    }
                }
        ...
        {
            if (string.IsNullOrEmpty(origem)) return 0;
        ...
        {
            if (string.IsNullOrEmpty(origem)) return 1;

            string sql = @"INSERT INTO tbOrigemDoacao (nome) VALUES (@nome); SELECT LAST_INSERT_ID();";
            using (var cmd = new MySqlCommand(sql, conn, trans))
            {
        ...
        private void CarregarDados()
        {
            ...
            using (var conn = DataBaseConnection.OpenConnection())
            {
                // Query corrigida para calcular saldo corretamente
                string sql = @"
            SELECT 
                l.codList,
                ...
                l.peso,
                COALESCE(
                    (SELECT SUM(quantidade) FROM tbProdutos p WHERE p.codList = l.codList AND p.quantidade > 0), 0
                ) AS total_entradas,
                COALESCE(
                    (SELECT SUM(ABS(quantidade)) FROM tbProdutos p WHERE p.codList = l.codList AND p.quantidade < 0), 0
                ) AS total_saidas,
            ...
            FROM tbLista l
            WHERE EXISTS (
                SELECT 1 FROM tbProdutos p WHERE p.codList = l.codList
            )";

                // Filtro por produtos principais (cesta básica)
                if (somentePrincipais)
                {
                    string produtosFiltro = string.Join("','", produtosPrincipais);
                    sql += $" AND l.descricao IN ('{produtosFiltro}')";
                }
                ...
                using (var cmd = new MySqlCommand(sql, conn))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            int entradas = Convert.ToInt32(reader["total_entradas"]);
                            int saidas = Convert.ToInt32(reader["total_saidas"]);
                            int qtd = entradas - saidas; // Saldo atual
        ...
        private void DiagnosticarEstoque()
        {
            using (var conn = DataBaseConnection.OpenConnection())
            {
                string sql = @"
            SELECT 
                l.descricao,
                COALESCE(SUM(CASE WHEN p.quantidade > 0 THEN p.quantidade ELSE 0 END), 0) AS entradas,
                COALESCE(SUM(CASE WHEN p.quantidade < 0 THEN ABS(p.quantidade) ELSE 0 END), 0) AS saidas,
            ...
            ORDER BY l.descricao";
                ...
                sb.AppendLine("Produto | Entradas | Saídas | Saldo");
                sb.AppendLine("----------------------------------------");
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        string produto = reader["descricao"].ToString();
                        ...
                        sb.AppendLine($"{produto} | {entradas} | {saidas} | {saldo}");
                    }
                }

                // Mostrar diagnóstico em uma MessageBox
                MessageBox.Show(sb.ToString(), "DIAGNÓSTICO DE ESTOQUE",
        ...
        {
            if (e.RowIndex >= 0 && dgvEstoque.Rows[e.RowIndex].Cells["Status"].Value != null)
            {
                string status = dgvEstoque.Rows[e.RowIndex].Cells["Status"].Value.ToString();

                if (status == "Vencido")
                    dgvEstoque.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.LightCoral;
                else if (status == "Próximo")
                    dgvEstoque.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.Khaki;
                else if (status == "Válido")
                    dgvEstoque.Rows[e.RowIndex].DefaultCellStyle.BackColor = Color.Honeydew;
            }
        }
        ...
        lblFiltroHistorico, txtFiltroHistorico, btnFiltrarHistorico, btnLimparFiltroHistorico
            ...
            // DataGridView com altura adequada
        ...
        private void ConfigurarTabRegistrarSaida()
        {
            ...
            tabPageRegistrarSaida.AutoScroll = false;

            // ===== GRUPO SELEÇÃO DO PRODUTO =====
            ...
            grpSelecao.Controls.AddRange(new Control[] {
        lblProduto, cmbProdutoSaida, lblSelecionado, lblProdutoSelecionado, lblSaldo, lblSaldoAtual
    });

            // ===== GRUPO DADOS DA SAÍDA =====
            ...
            grpSaida.Controls.AddRange(new Control[] {
        lblQtd, numQuantidadeSaida, lblDestino, txtDestinoSaida
    });
            ...
            tabPageRegistrarSaida.Controls.AddRange(new Control[] {
        grpSelecao, grpSaida, btnRegistrarSaida
    });
        }
        ...
        private void CmbProdutoSaida_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                // Calculando saldo diretamente da tabela tbProdutos
        ...
        private void RegistrarSaida(string produto, int quantidade, string destino)
        {
            ...
            using (var trans = conn.BeginTransaction())
            {
                try
                {
                    ...
                    // 2. Calcular saldo atual diretamente
                    string sqlSaldo = @"
                SELECT COALESCE(SUM(CASE WHEN quantidade > 0 THEN quantidade ELSE 0 END), 0) -
        ...
        private void btnSincronizarEstoque_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                using (var fileStream = File.Open(ofd.FileName, FileMode.Open, FileAccess.Read))
                {
                    using (var excelReader = ExcelReaderFactory.CreateReader(fileStream))
                    {
                        ...
                        var excelData = excelReader.AsDataSet(excelConfig);
                        DataTable planilhaDados = excelData.Tables[0];

                        // 🔍 DEBUG: Mostrar quantas linhas a planilha tem
                        MessageBox.Show($"Planilha carregada! Linhas: {planilhaDados.Rows.Count}\nColunas: {planilhaDados.Columns.Count}\n\nPrimeira coluna: {planilhaDados.Columns[0].ColumnName}\nSegunda coluna: {planilhaDados.Columns[1].ColumnName}");

                        int ajustesRealizados = 0;
                        StringBuilder logAjustes = new StringBuilder();
                        ...
                        using (var transacao = conexao.BeginTransaction())
                        {
                            try
                            {
                                foreach (DataRow linha in planilhaDados.Rows)
                                {
                                    // 🔍 DEBUG: Mostrar o que está lendo
                                    string nomeProduto = linha["Produto"]?.ToString();
                                    string qtdStr = linha["Qtd"]?.ToString();

                                    logAjustes.AppendLine($"Lendo: Produto='{nomeProduto}', Qtd='{qtdStr}'");

                                    if (string.IsNullOrEmpty(nomeProduto) || nomeProduto.Contains("TOTAL"))
                                        continue;

                                    if (nomeProduto.Contains("TOTAL GERAL"))
                                        break;

                                    if (!int.TryParse(qtdStr, out int quantidadeCorreta))
                                    {
                                        logAjustes.AppendLine($"  → Falha ao converter quantidade: '{qtdStr}'");
                                        continue;
                                    }

                                    logAjustes.AppendLine($"  → Quantidade convertida: {quantidadeCorreta}");

                                    // Buscar código do produto
                                    int codigoProduto = 0;
                                    string sqlBuscaProduto = "SELECT codList FROM tbLista WHERE descricao = @produto";
                                    using (var cmd = new MySqlCommand(sqlBuscaProduto, conexao, transacao))
                                    {
                                        cmd.Parameters.AddWithValue("@produto", nomeProduto);
                                        object codigoEncontrado = cmd.ExecuteScalar();
                                        if (codigoEncontrado == null)
                                        {
                                            logAjustes.AppendLine($"  → Produto NÃO encontrado no banco: {nomeProduto}");
                                            continue;
                                        }
                                        codigoProduto = Convert.ToInt32(codigoEncontrado);
                                        logAjustes.AppendLine($"  → Código encontrado: {codigoProduto}");
                                    }

                                    // Calcular saldo atual no banco
                                    string sqlCalculoSaldo = @"
                                SELECT COALESCE(SUM(CASE WHEN quantidade > 0 THEN quantidade ELSE 0 END), 0) -
                                       COALESCE(SUM(CASE WHEN quantidade < 0 THEN ABS(quantidade) ELSE 0 END), 0)
                                FROM tbProdutos WHERE codList = @codList";
                                    ...
                                    int saldoAtualBanco;
                                    using (var cmd = new MySqlCommand(sqlCalculoSaldo, conexao, transacao))
                                    {
                                        cmd.Parameters.AddWithValue("@codList", codigoProduto);
                                        object saldoObtido = cmd.ExecuteScalar();
                                        saldoAtualBanco = saldoObtido != null ? Convert.ToInt32(saldoObtido) : 0;
                                        logAjustes.AppendLine($"  → Saldo atual no banco: {saldoAtualBanco}");
                                    }

                                    int diferencaAjuste = quantidadeCorreta - saldoAtualBanco;
                                    logAjustes.AppendLine($"  → Diferença: {diferencaAjuste} (alvo: {quantidadeCorreta} - atual: {saldoAtualBanco})");

                                    if (diferencaAjuste != 0)
                                    {
                                        string sqlRegistrarAjuste = @"
                                    INSERT INTO tbProdutos 
                                        (descricao, quantidade, peso, unidade, dataDeEntrada, 
                                         dataDeValidade, tipoMovimentacao, codUsu, codOri, codList, destino)
                                    VALUES 
                                        (@descricao, @quantidade, 
                                         (SELECT peso FROM tbLista WHERE codList = @codList),
                                         (SELECT unidade FROM tbLista WHERE codList = @codList),
                                         NOW(), DATE_ADD(NOW(), INTERVAL 30 DAY),
                                         'AJUSTE', @codUsu, 1, @codList, 'SINCRONIZACAO PLANILHA')";

                                        using (var cmd = new MySqlCommand(sqlRegistrarAjuste, conexao, transacao))
                                        {
                                            cmd.Parameters.AddWithValue("@descricao", nomeProduto);
                                            cmd.Parameters.AddWithValue("@quantidade", diferencaAjuste);
                                            cmd.Parameters.AddWithValue("@codList", codigoProduto);
                                            cmd.Parameters.AddWithValue("@codUsu", codUsuLogado);
                                            cmd.ExecuteNonQuery();
                                        }
                                        ...
                                        ajustesRealizados++;
                                        logAjustes.AppendLine($"  → ✅ AJUSTE REALIZADO: {diferencaAjuste} unidades");
                                    }
                                    else
                                    {
                                        logAjustes.AppendLine($"  → ⏭️ Sem ajuste necessário");
                                    }

                                    logAjustes.AppendLine("---");
                                }
                                ...
                                string mensagemFinal = $"✅ Sincronização concluída!\n\n" +
                                                       $"Ajustes realizados: {ajustesRealizados}\n\n" +
                                                       $"LOG COMPLETO:\n{logAjustes}";

                                MessageBox.Show(mensagemFinal, "Sincronização de Estoque",
                                    MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            catch (Exception erro)
                            {
                                transacao.Rollback();
                                MessageBox.Show($"❌ Erro durante a sincronização: {erro.Message}\n\n{erro.StackTrace}");
                            }
                        }
                    }
                }
            ...
            {
                MessageBox.Show($"❌ Erro ao abrir arquivo: {erro.Message}\n\n{erro.StackTrace}");
            }
        }
        ...
        {
            // Ativar modo apenas produtos principais
            somentePrincipais = true;
            produtoSelecionado = "";

            // Limpar o combo e adicionar apenas os produtos principais
            cbxprodutoSelecionado.Items.Clear();
            ...
            // Recarregar os dados com o filtro
            CarregarDados();
        ...
        private void BtnExportar_Click(object sender, EventArgs e)
        {
            ...
            {
                // Está na aba de Histórico de Saídas
        ...
        private void ExportarHistoricoSaidasCSV()
        {
            ...
            try
            {
                ...
                MessageBox.Show($"✅ Histórico de Saídas exportado com sucesso!\n\nLocal: {sfd.FileName}",
        ...
        private void ImprimirEstoque()
        {
            ...
            printDocument.PrintPage += (sender, e) =>
            {
                ...
                e.Graphics.DrawString("Relatório de Estoque", tituloFont, Brushes.Black, e.MarginBounds.Left, yPos);
                yPos += 35;

                e.Graphics.DrawString($"Data: {DateTime.Now:dd/MM/yyyy HH:mm:ss}", subtituloFont, Brushes.Black, e.MarginBounds.Left, yPos);
                yPos += 30;

                float colX = e.MarginBounds.Left;
                float colWidth = (e.MarginBounds.Width) / dgvEstoque.Columns.Count;

                e.Graphics.FillRectangle(Brushes.LightGray, e.MarginBounds.Left, yPos, e.MarginBounds.Width, 25);
                for (int i = 0; i < dgvEstoque.Columns.Count; i++)
                {
                ...
                foreach (DataGridViewRow row in dgvEstoque.Rows)
                {
                    ...
                    if (isTotal)
                        e.Graphics.FillRectangle(Brushes.DarkGray, e.MarginBounds.Left, yPos, e.MarginBounds.Width, 22);
                ...
                e.Graphics.DrawString($"Total de registros: {dgvEstoque.Rows.Count - 1} itens", subtituloFont, Brushes.Gray, e.MarginBounds.Left, yPos + 10);
            };
        ...
        private void btnSincronizar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                MessageBox.Show("1 - Botão funcionou!");

                OpenFileDialog ofd = new OpenFileDialog();
                ofd.Filter = "Arquivos Excel|*.xlsx;*.xls";

                MessageBox.Show("2 - Dialog criado!");

                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    MessageBox.Show($"3 - Arquivo selecionado: {ofd.FileName}");
                }
                else
                {
                    MessageBox.Show("3 - Nenhum arquivo selecionado");
                }
            }
            catch (Exception ex)
            {
...
//                    string sqlSaldo = "SELECT quantidade FROM tbEstoqueItens WHERE codList = @codList";
//                    int saldoAtual = 0;
//                    using (var cmd = new MySqlCommand(sqlSaldo, conn, trans))
//                    {
//                        cmd.Parameters.AddWithValue("@codList", codList);
...
//        private void BtnImportar_Click(object sender, EventArgs e)
//        {
//            // Mantenha seu código de importação
//        }
...
//        private int ObterCodigoOrigem(MySqlConnection conn, MySqlTransaction trans, string origem) { return 1; }
//        private int InserirNovaOrigem(MySqlConnection conn, MySqlTransaction trans, string origem) { return 1; }
//        private int ObterPesoProduto(MySqlConnection conn, MySqlTransaction trans, int codList) { return 1000; }
//        private void InserirEntradaEstoque(MySqlConnection conn, MySqlTransaction trans, int codList, int codOri, string produto, int quantidade, int peso, DateTime dataEntrada, DateTime validade) { }
...
//    DateTime dataEntrada, DateTime validade)
//        {
//            // Só insere se quantidade for maior que ZERO
//            if (quantidade <= 0) return;
...
//            ConfigurarDataGridView();  // SEM PARÂMETRO
//            CarregarProdutos();
//            CarregarDados();
...
//        //        if (modoAgrupado)
//        //        {
//        //            // QUERY MODIFICADA: Remove o filtro "ei.quantidade > 0" para mostrar todos os produtos
//        //            string sql = @"
//        //        SELECT 
//        //            l.descricao AS produto, 
//        //            COALESCE(ei.quantidade, 0) AS quantidade,
//        //            l.unidade AS unidade,
//        //            l.peso,
//        //            (SELECT MIN(p2.dataDeValidade) 
//        //             FROM tbProdutos p2 
...
//        //        WHERE (@produto = '' OR l.descricao = @produto)
//        //        ORDER BY l.descricao";
...
//        //                            dgvEstoque.Rows[rowIndex].DefaultCellStyle.BackColor = Color.Honeydew;
//        //                        else
//        //                            dgvEstoque.Rows[rowIndex].DefaultCellStyle.BackColor = Color.White;
//        //                    }
//        //                }
//        //            }
//        //        }
//        //        else
//        //        {
//        //            // MODO DETALHADO: Mostra todos os lotes (positivos e negativos)
//        //            string sql = @"
//        //        SELECT p.codProd codigo,
//        //               l.descricao produto,
//        //               l.peso,
...
//        //                        if (quantidade <= 0)
//        //                        {
//        //                            dgvEstoque.Rows[rowIndex].DefaultCellStyle.BackColor = Color.LightGray;
//        //                        }
...
//                if (data is DateTime)
//                    dataConvertida = (DateTime)data;
//                else if (DateTime.TryParse(data.ToString(), out dataConvertida))
...
//        //            // 2. Verificar estoque disponível - USE tbEstoqueItens, NÃO tbProdutos
//        //            string sqlSaldo = "SELECT quantidade FROM tbEstoqueItens WHERE codList = @codList";
//        //            int saldoAtual = 0;
//        //            using (var cmd = new MySqlCommand(sqlSaldo, conn, trans))
//        //            {
...
//        //            // 2. Verificar estoque disponível na tabela tbEstoqueItens
//        //            string sqlSaldo = "SELECT quantidade FROM tbEstoqueItens WHERE codList = @codList";
//        //            int saldoAtual = 0;
//        //            using (var cmd = new MySqlCommand(sqlSaldo, conn, trans))
//        //            {
...
//        //            // 3. Buscar registros mais antigos para FIFO (apenas os que têm quantidade > 0)
//        //            string sqlBuscar = @"
//        //        SELECT codProd, quantidade 
//        //        FROM tbProdutos 
//        //        WHERE codList = @codList AND quantidade > 0 
...
//        //            // 7. Atualizar o estoque total na tabela tbEstoqueItens
//        //            string sqlUpdateEstoque = @"
//        //        UPDATE tbEstoqueItens 
//        //        SET quantidade = quantidade - @qtd,
//        //            dataMovimentacao = CURDATE(),
...
//        //                    string sqlInsertEstoque = @"
//        //                INSERT INTO tbEstoqueItens (codList, quantidade, dataMovimentacao, horaMovimentacao)
//        //                VALUES (@codList, @qtd, CURDATE(), CURTIME())";
//        //                    using (var cmdInsert = new MySqlCommand(sqlInsertEstoque, conn, trans))
//        //                    {
//        //                        cmdInsert.Parameters.AddWithValue("@codList", codList);
//        //                        cmdInsert.Parameters.AddWithValue("@qtd", -quantidade);
//        //                        cmdInsert.ExecuteNonQuery();
//        //                    }
//        //                }
//        //            }
...
//            // Verificar estoque atual ANTES de tentar registrar
//            int saldoAtual = 0;
//            using (var conn = DataBaseConnection.OpenConnection())
//            {
//                string sqlSaldo = @"
...
//                using (var cmd = new MySqlCommand(sqlSaldo, conn))
//                {
//                    cmd.Parameters.AddWithValue("@produto", produto);
//                    object result = cmd.ExecuteScalar();
//                    saldoAtual = result != null ? Convert.ToInt32(result) : 0;
